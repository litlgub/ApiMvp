...
function displayMovies(responseJSON){
  ...
  for (i=0; i<responseJSON.Similar.Results.length; i++){
    $('.results').append(`<h2 class="title">${responseJSON.Similar.Results[i].Name}</h2> <a class="trailer" href="${responseJSON.Similar.Results[i].yUrl}">Watch Trailer</a> <div id="${responseJSON.Similar.Results[i].yID}"></div>`);
...
function displayRatings(responseJSON, movieID){
  ...
  if (responseJSON.Plot=='N/A'){
    $(`#${movieID}`).append(`<h3 class="error">No other info available</h3>`);
      ...
      $(`#${movieID}`).append(`<img src="${responseJSON.Poster}"><p class="ratings">${responseJSON.Plot}<br /><br /><span class="important">IMDB:</span> ${responseJSON.Ratings[0].Value}<br /><span class="important">${responseJSON.Ratings[1].Source}:</span> ${responseJSON.Ratings[1].Value}<br /><span class="important">Year:</span> ${responseJSON.Year} <span class="important">Length:</span>${responseJSON.Runtime}<br /><span class="important">Starring:</span> ${responseJSON.Actors}</p>`);
  }
